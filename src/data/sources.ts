...
export const MODEL_EXPLAINERS: ModelExplainer[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      'The map shows the change in the balance divided by that country\u2019s own year-to-year spread, in standard deviations \u2014 the idea behind SPEI. Rainfall variability rises steeply as the mean falls, from about 15% of the mean in wet maritime climates to most of it in a desert, so the same 50 mm is noise in Ireland and a catastrophe in Libya. Read it in tenths: this is a permanent shift of the whole distribution, not one bad season, and moving the long-term mean by a full standard deviation takes roughly 4\u00b0C of warming. Half of one is already enough to roughly double how often a country meets what is currently its one-year-in-six drought.',
  ...
  },
  {
    ...
    steps: [
      ...
      'Then the people. Each country carries a population, shared with the fire model, and is counted into the band its yearly peak falls in. Where the measured share of the population living below 5 m is large, the peak is weighted toward the coastal strip, which the sea reaches in full \u2014 the only piece of sub-national information this app has, and it matters, because the wet-bulb extremes are coastal and so is most of the exposed population. It is still a whole country\u2019s population bucketed from a single national number split two ways: it changes which band a country lands in for three countries in 241 \u2014 Oman, Iraq and Egypt, whose people are on a coast their centroid is nowhere near \u2014 and it says how many people live somewhere that now reaches a line on its worst day, not how many people stand in it.',
    ],
    formula:
      'Tw = f_Stull(T, RH); RH from vapour = max(land evaporation, sea advection); days above X = \u03a3 months \u00d7 P(daily max Tw > X)',
    limits:
      'This cannot tell you about a city. It is one number per country evaluated at the country\u2019s middle, and humid heat varies enormously inside a country: Basra and Baghdad are 5\u00b0C of wet bulb apart, Jacobabad and the Balochistan uplands more than that, and Alexandria and Aswan more again \u2014 so Iraq, Pakistan and Egypt all read low here because their centroids miss the river valley where the people and the humidity are. It has no altitude, exactly as the water-balance layer does not, so Ethiopia, Bolivia, Afghanistan, Mongolia, Kazakhstan and western China come out several degrees too hot and rank far higher than they should; the Sahara and Sahel are the other weak region, roughly 1\u00b0C high \u2014 the model puts the Saharan countries at 25 to 27\u00b0C and the Sahel near 28\u00b0C where the observed peaks are 23 to 27\u00b0C \u2014 because the shared temperature climatology runs hot along the desert\u2019s southern margin. There is no acclimatisation in it and no air conditioning: a wet bulb is a property of the air, not of the person in it, and a Gulf state at 33\u00b0C with universal cooling and an Indian state at 31\u00b0C without it are not in the same danger. Nor is there humidity data \u2014 the humidity is inferred from a water balance and a prescribed sea surface temperature, and the day-to-day spread is a normal distribution fitted to reason rather than to observations, which will overstate the far tail because real wet bulbs have a hard convective ceiling that a normal curve does not. The population figures are country totals placed in the band of a national peak. The error is not evenly spread, and an average of it hides that rather than describing it. Checked against published national maxima the typical miss is around 2\u00b0C, but the small overall bias is the average of large errors pointing in opposite directions and cancelling: the Gulf and Red Sea run 3 to 4\u00b0C low, north-west Europe about 2\u00b0C low, the monsoon and humid-tropical countries about 1\u00b0C low, while the dry continental interiors and the Sahara and Sahel run about 1\u00b0C high. The worst single misses are all in one place and all in one direction \u2014 Saudi Arabia about 7\u00b0C low, and Iran, Yemen, Somalia and Djibouti 4 to 6\u00b0C low \u2014 because every one of those countries has its extreme humid heat on a coast and its centroid a long way inland, so the marine air never reaches the point the model evaluates. Read a low number in the Gulf, the Red Sea or the Horn as a floor rather than as an estimate.',
  },
  {
    id: 'rivers',
    title: 'River flow',
